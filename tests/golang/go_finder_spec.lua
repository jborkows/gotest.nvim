...
describe("parsing example", function()
	...
	it("test found second case", function()
		...
		assert(testB == 10, "Should found test B")
	end)
...
describe("find package name", function()
...
describe("finding from inside", function()
	it("should not find key outside function", function()
		vim.cmd("e tests/golang/example.go")
		local line = 10
		local column = 1
		local key = golang.findTestKey(line, column)
		assert.equals(true, key == nil)
	end)
	it("should find key inside from function", function()
		vim.cmd("e tests/golang/example.go")
		local line = 8
		local column = 1
		local key = golang.findTestKey(line, column)
		assert.equals("TestA", key.testName)
		assert.equals("apackage", key.packageName)
	end)
